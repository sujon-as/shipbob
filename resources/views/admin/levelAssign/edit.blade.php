...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Edit Assign VIP Level</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{URL::to('/dashboard')}}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{URL::to('/level-assign')}}">All Assign VIP Level
                                </a></li>
                        <li class="breadcrumb-item active">Edit Assign VIP Level</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="card card-success">
            <div class="card-header">
                <h3 class="card-title">Edit Assign VIP Level</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
                                ...
                                <select class="form-control select2bs4" name="user_id" id="user_id" required="">
                                    ...
                                    @if(count($users) > 0)
                                        @foreach ($users as $user)
                                            <option value="{{ $user['id'] }}" @if($user['id'] === $levelAssign->user_id) selected @endif>{{ $user['username'] }} ({{ $user['uid'] }})</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('user_id')
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control select2bs4" name="status" id="status" required>
                                    ...
                                    <option value="Active" @if($levelAssign->status === 'Active') selected @endif>Active</option>
                                    <option value="Inactive" @if($levelAssign->status === 'Inactive') selected @endif>Inactive</option>
                                </select>
                                @error('status')
                                <span class="alert alert-danger">{{ $message }}</span>
                                @enderror
